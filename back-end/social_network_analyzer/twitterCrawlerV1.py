...
class MyListener(StreamListener):
    ...
    def on_data(self, data):
        try:
            ...
            # handle twitterQuery1 collection
            twitterQuery1 = db1.twitterQuery1
            ...
            # insert the data into the mongoDB into a collection called twitterQuery1
            # if twitter_search doesn't exist, it will be created.
            print('Inserting to MongoDB..')
            db1.twitterQuery1.insert(datajson)
...
if __name__ == '__main__':
    ...
    markets = ['#BugBounty', '#vulnerability #market', '#security #contest', '#bugsbounty', '#securitycontest',
               '#securitycontest #vulnerability', '#vulnerability #bugsbounty', '#bugbounty #securitycontest',
               '#securitycontest #bugsbounty']

    # parser = get_parser()
    # args = parser.parse_args()
    auth = OAuthHandler(config.consumer_key[1], config.consumer_secret[1])
    auth.set_access_token(config.access_token[1], config.access_secret[1])
    api = tweepy.API(auth)
    ...
    twitter_stream.filter(track=markets)
